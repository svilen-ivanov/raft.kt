...
func NewRaft(conf *Config, fsm FSM, logs LogStore, stable StableStore, snaps SnapshotStore, trans Transport) (*Raft, error) {
    ...
    if conf.Logger != nil {
        ...
        logger = hclog.New(& hclog . LoggerOptions {
            ...
            DocFinder.Output: conf.LogOutput,
        })
    }
...
func (r *Raft) BootstrapCluster(configuration Configuration) Future {
    ...
    select {
        case < -r.shutdownCh:
        return errorFuture { Error.ErrRaftShutdown }
...
func (r *Raft) ApplyLog(log Log, timeout time.Duration) ApplyFuture {
    ...
    select {
        case < -timer:
        return errorFuture { Error.ErrEnqueueTimeout }
        case < -r.shutdownCh:
        return errorFuture { Error.ErrRaftShutdown }
...
func (r *Raft) AddVoter(id ServerID, address ServerAddress, prevIndex uint64, timeout time.Duration) IndexFuture {
    ...
    return r.requestConfigChange(configurationChangeRequest { command: AddStaging,
                                                              serverID: id,
...
func (r *Raft) AddNonvoter(id ServerID, address ServerAddress, prevIndex uint64, timeout time.Duration) IndexFuture {
    ...
    return r.requestConfigChange(configurationChangeRequest { command: AddNonvoter,
                                                              serverID: id,
                                                              serverAddress: address,
                                                              prevIndex: prevIndex,
    }, timeout)
...
func (r *Raft) RemoveServer(id ServerID, prevIndex uint64, timeout time.Duration) IndexFuture {
    ...
    return r.requestConfigChange(configurationChangeRequest { command: RemoveServer,
                                                              serverID: id,
                                                              prevIndex: prevIndex,
    }, timeout)
}
...
func (r *Raft) DemoteVoter(id ServerID, prevIndex uint64, timeout time.Duration) IndexFuture {
    ...
    return r.requestConfigChange(configurationChangeRequest { command: DemoteVoter,
                                                              serverID: id,
                                                              prevIndex: prevIndex,
    }, timeout)
}
...
func (r *Raft) Restore(meta *SnapshotMeta, reader io.Reader, timeout time.Duration) error {
    ...
    // Perform the restore.
    restore : = &userRestoreFuture{ meta: meta,
                                    reader: reader,
    }
    restore.init()
    select {
